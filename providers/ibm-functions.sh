...
instance_pretty() {
 ...
 hour_cost=0
 for f in $(echo $data | jq -r '.[].billingItem.hourlyRecurringFee'); do
  new=$(bc <<<"$hour_cost + $f")
  hour_cost=$new
 done
 ...
 hours_used=0
 for f in $(echo $data | jq -r '.[].billingItem.hoursUsed'); do
  new=$(bc <<<"$hours_used + $f")
  hours_used=$new
 done
 ...
 monthly_cost=0
 for f in $(echo $data | jq -r '.[].billingItem.orderItem.recurringAfterTaxAmount'); do
  new=$(bc <<<"$monthly_cost + $f")
  monthly_cost=$new
 done
