...
_ax_completions()
{
    ...
    local file_completion_commands="account account build configure deploy exec fleet images init ls power provider region rm scan scp select ssh sync update"
    ...
    if [[ " ${file_completion_commands} " =~ " ${command} " ]]; then
        ...
        local commands="account account build configure deploy exec fleet images init ls power provider region rm scan scp select ssh sync update"
